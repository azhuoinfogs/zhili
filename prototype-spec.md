...
**与仓库同步（快照）**：2026-05-04 — H5/API/算法/埋点路径与下文一致；**`products.json` 商品条数：200**；列表支持 **下拉刷新、触底分页**；详情 **多图 + 类似推荐** 见 API。**MVP B1**：`POST /api/user/login`、`GET /api/user/me`（JWT）；详见 **`server/.env.example`** 与 [prototype/README.md](prototype/README.md)「B1」。**B2**：**`user_profile` 表**（策略 A）+ **`/api/profile*`** REST 已实现；契约见 **[api.md](../api.md) §2、§4.3**。**B3**：**`GET /api/user/recommend`**（默认画像 + `recommendCore` 与 hot/personalized 同源）；见 **[api.md](../api.md) §4.2.1**。**B4**：**`GET /api/recommend`**（`page`/`size` + Redis 可降级 + 画像写后失效）；见 **[api.md](../api.md) §4.2.2**。整体排期见 **[develop2.md](develop2.md)** 篇首与 **附录 A**。
