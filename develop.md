...
| **F2** | 双列流；顶筛场合/预算/风格；**下拉刷新、上拉更多**；防抖 500ms；空状态+清空 | 双列✅；粘性筛选✅；防抖✅；空状态文案+清空✅；**无下拉/上拉分页** | 双列+请求；筛选以页面为准；**无刷新/分页** |
| **F3** | 图轮播 **≥3**；理由 2–3 条；规格折叠；**横向类似推荐**；底栏收藏+去购买 | 单主图；理由来自算法✅；**无轮播/规格/类似推荐**；底栏在抽屉内✅ | 详情占位为主 |
...
优先级：**不改变 A/B 主指标前提下**，缩小 PRD 与演示差距，便于评审与录屏。

| 序号 | 任务 | PRD 锚点 | 验收 |
|------|------|-----------|------|
| A1 | 列表 **下拉刷新**、**触底加载**（分页参数与 API 约定） | F2 | 刷新重拉；加载追加且不重复 |
| A2 | 详情 **swiper 多图**（数据缺省时用同图占位或 seed 多 URL） | F3 | ≥1 张可滑，数据结构兼容 ≥3 |
| A3 | 详情 **横向类似推荐**（同场合或同兴趣召回 4–8 条） | F3 | 可点击跳转详情 |
| A4 | 空状态 **插画或品牌图**（静态资源即可） | F2 | 比纯文案更符合 §5.1 体验描述 |
| A5 | 商品池 **扩至 PRD 建议 200**（脚本与抽检标签） | 4.2、§10 v1.2 前置 | `gen:products` 或运营表导入 |
